...
class RayPPOTrainer:
    ...
    def _create_dataloader(self, train_dataset, val_dataset, collate_fn, train_sampler):
        ...
        if train_dataset is None:
            train_dataset = create_rl_dataset(self.config.data.train_files, self.config.data, self.tokenizer, self.template_tokenizer, self.processor)
        if val_dataset is None:
            val_dataset = create_rl_dataset(self.config.data.val_files, self.config.data, self.tokenizer, self.template_tokenizer, self.processor)
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            prompt_lengths.extend((input_ids != self.tokenizer.pad_token_id).sum(dim=-1).cpu().tolist())
            ...
            response_lengths.extend((output_ids != self.tokenizer.pad_token_id).sum(dim=-1).cpu().tolist())
        ...
        metric_dict["response_length/val_response_length/mean"] = np.mean(response_lengths).item()
        metric_dict["response_length/val_response_length/min"] = np.min(response_lengths).item()
        metric_dict["response_length/val_response_length/max"] = np.max(response_lengths).item()
        metric_dict["prompt_length/val_prompt_length/mean"] = np.mean(prompt_lengths).item()
        metric_dict["prompt_length/val_prompt_length/min"] = np.min(prompt_lengths).item()
        metric_dict["prompt_length/val_prompt_length/max"] = np.max(prompt_lengths).item()
        return metric_dict
