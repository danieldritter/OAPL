...
def default_compute_score(data_source, solution_str, ground_truth, extra_info=None, sandbox_fusion_url=None, concurrent_semaphore=None):
    ...
    """
    # removing thinking tokens if present
    # solution_str = re.sub(r"<think>.*?</think>\n?\n?", "", solution_str, flags=re.DOTALL)
    if data_source == "openai/gsm8k":
        ...
        "math-ai/minervamath",
        "VGS-AI/OpenR1-Cleaned",
        "DigitalLearningGmbH/MATH_HARD-lighteval",
    ...
    elif data_source in ["taco", "apps", "code_contests"]:
        from . import rllm_code

        res = rllm_code.compute_score_taco_apps_codecontests(solution_str, ground_truth, continuous=False)
    elif data_source in ["livecodebench", "codeforces", "primeintellect", "lcb_agentica-org/DeepCoder-Preview-Dataset", "ojo2/deepcoder_gen", "deepcoder_round2"]:
        from . import rllm_code

        res = rllm_code.compute_score_lcb(solution_str, ground_truth, continuous=False)
    ...
    # res = prime_code.compute_score(solution_str, ground_truth, continuous=True)
        ...
        # Fallback to math_verify for unsupported data sources
        res = math_verify.compute_score(solution_str, ground_truth)
