...
class ActorRolloutRefWorker(Worker):
    ...
    def __init__(self, config: DictConfig, role: str):
        ...
        if not torch.distributed.is_initialized():
            ...
            torch.distributed.init_process_group(backend="cpu:gloo,cuda:nccl" if is_cuda_available else "cpu:gloo,npu:hccl", rank=rank, world_size=world_size, timeout=timedelta(hours=24))
    ...
    def _build_model_optimizer(
        ...
        if role == "actor" and optim_config is not None:
            ...
            betas = (beta1, beta2)
            if optim_config.get("optimizer_type", "adamw").lower() == "adamw":
            ...
            elif optim_config.get("optimizer_type", "adamw").lower() == "decoupled_adamw":
                actor_optimizer = DecoupledAdamW(
                    actor_module_fsdp.parameters(),
                    lr=optim_config.lr,
                    betas=betas,
...
class CriticWorker(Worker):
    def __init__(self, config):
        ...
        if not torch.distributed.is_initialized():
            torch.distributed.init_process_group(backend="nccl" if is_cuda_available else "hccl", timeout=timedelta(hours=24))
        self.config = config
